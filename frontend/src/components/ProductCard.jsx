...
const ProductCard = ({ product }) => {
  ...
  const addToCartHandler = (e) => {
    ...
    // For simplicity, adding to cart without customizations from this card.
    // A modal could be added here in the future for customization selection.
    const newItem = {
        product: product._id,
        name: product.name,
        imageUrl: product.imageUrl,
        price: product.basePrice,
        selectedCustomizations: {}, // Default empty
  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-2xl transition-shadow duration-500 ease-in-out group relative">
        ...
        <div className="relative overflow-hidden h-[32rem]">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-500 flex items-center justify-center">
              <div className="flex items-center space-x-4 transform translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <button onClick={addToCartHandler} className="bg-white text-slate-900 font-bold py-3 px-6 rounded-md text-sm uppercase tracking-wider hover:bg-slate-200 transition-colors">
                      Add to Cart
                  </button>
                  <button 
                    onClick={(e) => {
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 transition-colors ${isFavorited ? 'text-red-500' : 'text-slate-700'}`} fill={isFavorited ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
